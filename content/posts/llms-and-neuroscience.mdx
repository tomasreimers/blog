...
The same is true of memory, reasoning, or generalization. It doesn't matter if an LLM's memory is as good as a human's or if it can reason as well. 

All that matters is we have a system for memory that: 
1. Gives us non-random distributions over the results
2. We can scale to be arbitrarily large

So long as it's non-random, scale is all you need. 
